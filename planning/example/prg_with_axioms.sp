...
-occurs(serve(R,X,P),I) :- holds(status(X,damaged),I), not has_role(P,engineer).
...
-occurs(pickup(R,X),I) :- has_weight(X,heavy) , has_arm_type(R,electromagnetic).


%%(6) "An item with item_status 'damaged' cannot be labelled by a robot with a pneumatic."
%%-occurs(affix_label(R,X),I) :- holds(status(X,damaged),I), has_arm_type(R,pneumatic).
...
holds(status(book1, damaged),0).
holds(is_labelled(book1, true),0).
holds(status(prin1, intact),0).
holds(is_labelled(prin1, true),0).
holds(loc(cup1, office),0).
holds(loc(book1, workshop),0).
holds(loc(prin1, workshop),0).
holds(loc(rob1, library),0).
holds(loc(p0, kitchen),0).
holds(loc(p1, kitchen),0).
holds(loc(p2, library),0).
